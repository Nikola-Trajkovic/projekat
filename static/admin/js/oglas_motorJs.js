    ...
    fetch('http://localhost:10000/api/oglas_motor', {
        ...
        .then( data => {
            ...
            data.forEach( el => {
                ...
                for(var i=0; i<oglasi.length; i++){
                    ...
                    if(oglasi[i].id == el.motorId){
                        naziv = oglasi[i].marka + " " + oglasi[i].model;
                    }
    
                }
                lst.innerHTML += `<li class="lstUser" onclick="update(this)">ID: ${el.id}, Opis: ${el.opis},Likes: ${el.likes}, Dislikes: ${el.dislikes},Contact: ${el.contact}, Cena: ${el.cena},MotorID: ${el.motorId},Motor: ${naziv} ,UserID: ${el.user.id},Usename: ${el.user.username}  <button class="liBtn" id="liBtn" onclick="obrisi(this)">X</button></li>`;
            });
        });
        ...
        document.getElementById("unesiBtn").addEventListener('click', e => {
            ...
            const data = {
                ...
                motorId: document.getElementById('cars').value
            };
